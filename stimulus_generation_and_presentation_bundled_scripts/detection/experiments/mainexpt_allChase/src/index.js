...
import metadata from "./all_videos_allchase.js"; // all trial IDs 
import stimSets from './stimSets_allChase.json'; // list of 500 combinations  of trials (in integers)
...
/** For the first exp */
/** Importing stimset files */
stimSets = stimSets.file_ind;
console.log(stimSets.length) // should be 500
// randomly selecting a row as stimset for each participant
...
function checkTimeAndEndExperiment() {
    ...
    if (lastTwoTrialsData.length === 2) {
        ...
        if (timeDifference > 15000) {
            ...
            jsPsych.endExperiment("<p style='width:1000px;'>The animation took over 15s to play out - with the current internet speed, you will not be able to complete this task. Please exit the study and message the experimenter providing details to be compensated for your time.</p>");
...
var quitting_message = {
    ...
    on_finish: function() {
        ...
        jsPsych.endExperiment('Missed/wrong response the second time. You are disqualified from taking part in this study.');
    }
};
...
var fr_q1 = {
    ...
    html: '<p>Please estimate the number of <b>close friends</b> that you have,' +
        ...
        '<p><input id = "friend" type="number" size="50%" height="50" name="friend" required/></p>',
}
...
for (let i = 0; i < survey_shuffled.length; ++i) {
    if (i > 0) { // display an intro page for later surveys
        timeline.push({
            type: surveyHtmlForm,
            preamble: "Moving on to the next questionnaire (#" + String(i + 1) + " of 5).<br>Keep it up!<p>Please take note of the new task instructions and the slight change in the response choices before you start answering questions.<br><br></p>",
            response_ends_trial: true,
            html: ' ',
            button_label: "Continue",
            extensions: [
...
/** Katie's question */
var control_pic = '../src/Intro/demo_img/control.jpg';
var condition_pic = '../src/Intro/demo_img/condition.jpg';

var selected_pic = Math.random() < 0.5 ? control_pic : condition_pic;

console.log(selected_pic);
timeline.push({
    type: surveyText,
    preamble: `<img src="${selected_pic}" style="width: 800px;"></img>`,
    questions: [{
        prompt: "You're on chaperone duty at the park, and it's the end of your shift. You want your replacement to keep an eye on the circled boy. You meet your replacement in the parking lot and ask them to keep an eye out on the boy who", 
        rows: 2}
    ],
    data: {
